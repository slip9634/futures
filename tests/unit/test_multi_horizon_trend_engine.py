...
from futures_quant.backtest.multi_horizon_trend_engine import run_multi_horizon_trend_backtest
...
def test_pure_uptrend_opens_one_long_and_closes_at_window_end():
    ...
    result = run_multi_horizon_trend_backtest(
        ...
        costs_config=_zero_cost_config(), scenario="base", lookbacks=LOOKBACKS,
    )
...
def test_too_short_series_produces_no_trades():
    closes = [100.0, 100.0, 100.0]  # shorter than longest lookback=3
    bars = _bar_series(closes)
    result = run_multi_horizon_trend_backtest(
        bars, root="MES", bar_size="1day", instrument=_mes_spec(),
        costs_config=_zero_cost_config(), scenario="base", lookbacks=LOOKBACKS,
...
def test_costs_reduce_net_pnl_versus_gross():
    ...
    result = run_multi_horizon_trend_backtest(
        bars, root="MES", bar_size="1day", instrument=_mes_spec(),
        costs_config=_real_cost_config(), scenario="base", lookbacks=LOOKBACKS,
    )
    trade = result.trades[0]
...
def test_stress_scenario_worse_than_base():
    closes = [float(i) for i in range(1, 21)]
    bars = _bar_series(closes)
    base = run_multi_horizon_trend_backtest(
        bars, root="MES", bar_size="1day", instrument=_mes_spec(),
        costs_config=_real_cost_config(), scenario="base", lookbacks=LOOKBACKS,
    )
    stress = run_multi_horizon_trend_backtest(
        bars, root="MES", bar_size="1day", instrument=_mes_spec(),
        costs_config=_real_cost_config(), scenario="stress", lookbacks=LOOKBACKS,
    )
